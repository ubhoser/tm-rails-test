...
private
  ...
  after_save do |campaign|
    ...
    if (@old_platform_ids != @current_plaform_ids && campaign.budget > 1000)
      CampaignHistory.create({:campaign_id => campaign.id, :admin_user_id => current_admin_user.id, :budget => campaign.budget})
    end
  end
